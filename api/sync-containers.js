'use strict';
/**
 * Routine to collect boxes from streamsideorganics.myshopify and store on mongo db
 * When storing on southbridge database replace ids with those on soutbridge-dev.myshopify.com
 *
 * Is a duplicate of sync-products
 *
...
const syncBoxContainers = async function (req, res, next) {
  const collection = req.app.locals.containerCollection;
  try {
    const shop = "SO";
    const path = "products.json";
    const fields = ["id", "title", "handle", "variants"];
...
module.exports = syncBoxContainers;
